...
class MultiOmicsDatasetSupervised(Dataset):
    def __init__(self, dna_data, gene_data, labels):
        """
        dna_data: tensor (N, input_dim_dna)
        gene_data: tensor (N, input_dim_gene)
        labels: tensor (N,) or (N, num_classes)
        """
        self.dna = dna_data
        self.gene = gene_data
        self.labels = labels
    ...
    def __getitem__(self, idx):
        return self.dna[idx], self.gene[idx], self.labels[idx]
...
def return_dataloaders_supervised(X_dna_meth, X_gene_exp, labels, split_fractions : tuple = (0.8, 0.1)):

    """ split_fractions (tuple) : fractional sizes of training and validation set """

    dataset = MultiOmicsDatasetSupervised(X_dna_meth, X_gene_exp, labels)
