...
def create_chromosome_filters(n_cpg: int, n_chromosomes: int = 23) -> np.ndarray:
    """
    Returns a (n_chromosomes, n_cpg) numpy array with:
      - each column corresponding to a CpG site
      - exactly one '1' per column (assigned chromosome)
      - no chromosome with 0 CpGs
    """
    if n_cpg < n_chromosomes:
        raise ValueError("n_cpg must be >= n_chromosomes so each chromosome gets at least one CpG.")

    assignments = np.arange(n_chromosomes)

    remaining = n_cpg - n_chromosomes
    if remaining > 0:
        extra = np.random.randint(0, n_chromosomes, size=remaining)
        assignments = np.concatenate([assignments, extra])

    np.random.shuffle(assignments)

    chrom_filt = np.zeros((n_chromosomes, n_cpg), dtype=np.float32)
    for cpg_idx, chrom_idx in enumerate(assignments):
        chrom_filt[chrom_idx, cpg_idx] = 1.0
...
def create_synthetic_data(
    ...
    chrom_filters = create_chromosome_filters(n_cpg=n_cpg, n_chromosomes=23)

    return dna_meth, gene_exp, chrom_filters


def build_model(
    input_dim_dna: int,
    input_dim_gene: int,
    chromosome_filters: np.ndarray,
    ...
    model = PVAE2(
        ...
        chromosome_filters=chromosome_filters,
        hls_dna=hls_dna,
        hls_gene=hls_gene,
...
def main():
    ...
    dna_meth_np, gene_exp_np, chrom_filters = create_synthetic_data(
        n_samples=n_samples,
        n_cpg=n_cpg,
        n_genes=n_genes,
        seed=42,
    ...
    model = build_model(
        ...
        chromosome_filters=chrom_filters,
        use_self_attn=False,  
    )
